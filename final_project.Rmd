...
g <- g + geom_histogram(aes(colour="Simulation",y=..density..), bins=40, col="black", fill="darkgreen", alpha=.3)
...
g <- g + stat_function(aes(colour="Theoretical"), fun = dnorm, 
                       args = list(mean=1/lambda, sd=1/(lambda*sqrt(n))), col="red")
g <- g + scale_colour_manual("title", values = c("darkgreen", "red"))
g
```